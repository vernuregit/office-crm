...
const normalizeStatus = (s) => {
  if (!s || s === "pending" || s === "to_do") return "to-do";
        ...
        <form onSubmit={handleSubmit} className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all bg-white"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              className="w-full border border-gray-200 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all bg-white resize-none"
            />
          </div>
          ...
          <div className="flex gap-3 pt-1">
            ...
            <button
              type="submit"
              disabled={saving}
              className="flex-1 py-2.5 rounded-xl bg-gradient-to-r from-indigo-500 to-purple-600 text-white text-sm font-semibold hover:opacity-90 transition-all shadow-md disabled:opacity-50 flex items-center justify-center gap-2"
            >
              {saving ? (
                <>
...
const TaskModal = ({ task, onClose, onStatusChange, onAddNote, userData }) => {
  ...
  const handleStatusChange = async (newStatus) => {
    setUpdating(true);
    const updatedTask = await onStatusChange(localTask.id, newStatus);
    if (updatedTask) {
      setLocalTask(updatedTask);
    } else {
      setLocalTask((prev) => ({ ...prev, status: toFirestoreStatus(newStatus) }));
    }
    setUpdating(false);
  };
  ...
  const handleAddNote = async () => {
    ...
    const newNote = await onAddNote(localTask.id, note.trim(), userData?.name);
    if (newNote) {
      setLocalTask((prev) => ({
        ...prev,
        ...
        <div className="sticky top-0 bg-white border-b border-gray-100 px-6 py-4 rounded-t-3xl flex items-start justify-between z-10">
          <div className="flex-1 pr-4">
            <div className="flex items-center gap-2 mb-1.5 flex-wrap">
              <span
                className={`text-xs font-semibold px-2.5 py-0.5 rounded-full border ${pConfig.bg} ${pConfig.color} ${pConfig.border}`}
              >
                <Flag size={10} className="inline mr-1" />
                {localTask.priority || "low"} priority
              </span>
              <span className="text-xs bg-gray-50 text-gray-500 border border-gray-200 px-2.5 py-0.5 rounded-full font-semibold">
                <Tag size={10} className="inline mr-1" />
                {localTask.category || "General"}
              </span>
            </div>
    ...
    >
      <div className="flex items-center justify-between mb-2.5">
        <div className="flex items-center gap-1.5 min-w-0">
          <GripVertical
            size={13}
            className="text-gray-300 flex-shrink-0 group-hover:text-indigo-300 transition-colors"
          />
        ...
        <div
          className={`flex items-center gap-1 text-xs font-semibold px-2 py-0.5 rounded-full ${pConfig.bg} ${pConfig.color} border ${pConfig.border} flex-shrink-0`}
        >
          <Flag size={9} />
          {task.priority || "low"}
        </div>
      ...
      {normalizedStatus === "review" && (
        <div className="flex items-center gap-1 mb-3">
          <span className="text-xs text-sky-500 font-medium bg-sky-50 px-2 py-0.5 rounded-full border border-sky-100">
            In review
          </span>
        </div>
      )}
...
const KanbanColumn = ({
  ...
  return (
    <div className="flex flex-col px-2">
      <div
        className={`flex items-center justify-between px-4 py-2.5 rounded-xl border mb-3 transition-all
      ...
      >
        <div className="space-y-3">
          {tasks.length === 0 ? (
            <div
              className={`flex flex-col items-center justify-center py-10 text-xs font-medium gap-2 rounded-xl border border-dashed transition-all
              ${
                isDragOver
            ...
            >
              <col.Icon
                size={20}
                className={isDragOver ? "text-indigo-300" : "text-gray-300"}
              />
              {isDragOver
                ? `Drop here → ${col.label}`
...
const MyTasks = () => {
  ...
  const handleDrop = async (taskId, newColumnKey) => {
    ...
    try {
      ...
      updateTaskInState(taskId, { status: task.status, updatedAt: task.updatedAt });
    }
  };
  ...
  const handleStatusChange = async (taskId, newKanbanStatus) => {
    ...
    const updatedTask = updateTaskInState(taskId, {
      status: firestoreStatus,
      updatedAt: { seconds: Math.floor(Date.now() / 1000) },
    });
  ...
  const counts = {
    ...
    inProgress: tasks.filter((t) => normalizeStatus(t.status) === "in-progress")
      .length,
  ...
  return (
    <Layout title="My Tasks">
      <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-6 px-5 mt-5 gap-4">
        <div className="grid grid-cols-2 xl:grid-cols-5 gap-4 flex-1">
          {[
            {
              label: "Total Tasks",
              val: counts.total,
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 px-4 py-2 bg-[#1D7872] cursor-pointer text-white rounded-xl text-sm font-bold hover:opacity-90 transition-all shadow-md"
        >
          <Plus size={16} />
      ...
      <div className="relative mb-5 py-5 px-5">
        ...
        <input
          type="text"
          placeholder="Search tasks, clients, category…"
          value={search}
          onChange={(e) => setSearch(e.target.value)}
          className="w-full bg-white border border-gray-200 rounded-xl pl-11 pr-4 py-2.5 px-10 text-sm text-gray-800 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-indigo-300 focus:border-indigo-400 transition-all shadow-sm"
        />
      </div>

      {!loading && tasks.length > 0 && (
        <div className="flex items-center gap-2 text-xs text-gray-400 font-medium mb-4 px-5">
      ...
      {loading ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-5 px-2">
          {[...Array(4)].map((_, i) => (
